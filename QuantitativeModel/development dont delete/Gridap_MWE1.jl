...
# 1. Define the mesh
model = GmshDiscreteModel("unit_circle_v0.3.msh")        # This is a unit circle mesh with 456370 elements
# model = GmshDiscreteModel("unit_circle_v0.1.msh")       # This is a unit circle mesh with 4928 elements
...
# 7. Define weak form (variational formulation)
a(u,v) = ∫( ∇(v) ⋅ ∇(u))dΩ
l(v)   = ∫( - real( (∇(v) ⋅ VectorValue(1im, 1.0)) *  C_fn ) )dΩ

# 8. Assemble and solve
op = AffineFEOperator(a, l, U, V)   # Very slow but it works.
Psi = Gridap.solve(op)

# 9. Visualization with Paraview: This works fine.
writevtk(Ω,"Gridap_MWE1_solution",cellfields=["Psi"=>Psi, "psurf"=>psurf])