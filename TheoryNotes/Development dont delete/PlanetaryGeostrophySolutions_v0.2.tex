...
\begin{align}
i f \mathfrak{U}(x) & = - H(x) \left(\frac{\partial}{\partial x} + i \frac{\partial}{\partial y} \right)
\left[ 
p_s(x) + \int_{-H(x)}^{0} b(x,z') \; dz'
\right] \nonumber \\
& - \left(\frac{\partial }{\partial y} + i \frac{\partial}{\partial x}  \right)  
\left[
 - \int_{-H(x)}^0 z' b(x,z') \; dz'
 \right]
+ \tau_s(x)  - \tau_b (x) .
 \end{align}
...
\mathrm{e}^{\sqrt{i} \varphi  \left( \xi + H(x) \right)} - 1
\right)
 \left( \partial_x  + i \partial_y  \right) p( \xi; x)  \; d \xi  \right. \nonumber \\
& - \left.
\left( 
\mathrm{e}^{\sqrt{i} \varphi  H(x) }   - 1   
\right)^2
\tau_s (x)  \right\} 
= - H(x)  \left( \partial_x  + i \partial_y  \right)
\left[ 
p_s(x) + \int_{-H(x)}^{0} b(x,z') \; dz'
\right] \nonumber \\
& -  \left( \partial_y  + i \partial_x  \right)
\left[
 - \int_{-H(x)}^0 z' b(x,z') \; dz'
 \right]
+ \tau_s (x)  - \tau_b (x) .
...
\frac{ \mathrm{e}^{-\sqrt{i} \varphi (\xi - H(x))  } }{ 1 + \mathrm{e}^{2 \sqrt{i} \varphi H \left(x \right)} }
\left( 1 + \mathrm{e}^{2 \sqrt{i} \varphi \xi}\right) 
 \left( \partial_x  + i \partial_y \right) p( \xi; x)  \; d \xi 
+\frac{ 2 \mathrm{e}^{\sqrt{i} \varphi H(x)  } }{ 1 + \mathrm{e}^{2 \sqrt{i} \varphi H \left(x \right)} }
\tau_s (x)
. \nonumber
\end{align}
Thus, substituting for the hydrostatic pressure from (\ref{eqn:hydrostatic_p_pgf}):
 \begin{align}
...
\right)^2
\tau_s (x)  \right\} 
 = \tau_s (x) - H(x) \left( \partial_x  + i \partial_y  \right)
\left[ 
p_s(x) + \int_{-H(x)}^{0} b(x,z') \; dz'
\right] \nonumber \\
& -  \left( \partial_y  + i \partial_x  \right)
...
\left[ 
p_s(x) + \int_{-H(x)}^{0} b(x,z') \; dz'
\right] +  \left( \partial_y  + i \partial_x  \right)
 \int_{-H(x)}^0 z' b(x,z') \; dz'
 \right]
\nonumber \\
&- 
...
. \nonumber
\end{align}
Hence, breaking apart the integrals:
 \begin{align}
& 
...
\nonumber \\
 & \int_{-H(x)}^{0} 
\left[ \mathrm{e}^{-\sqrt{i} \varphi \xi} 
\left(
\mathrm{e}^{\sqrt{i} \varphi \xi} - 
\mathrm{e}^{\sqrt{i} \varphi H(x)} 
\right) \left(
...
A(x, \xi)
 ...
 \; d \xi   \nonumber .
\end{align}  
This needs checking!  See \texttt{PlanetaryGeostrophicTheory\_v0.1.ipynb}.

This final expression is of the form
\begin{align}
 \left( \partial_x  + i \partial_y  \right) p_s (x) = \frac{C(x)}{D(x)} ,
 \label{eqn:ps_diffeq}
\end{align}
where $C(x)$ is given by $\tau_s(x), b(x, z), H(x)$ and $D(x)$ is given by $H(x)$.
The formal solution of (\ref{eqn:ps_diffeq}) is
\begin{align}
p_s(x,y) & = \int_0^{T(x,y)} \frac{C(x -t, y - i t)}{D(x -t, y - i t)} \; dt ,
\end{align}
or, equivalently (because $p_s$ is real)
\begin{align}
\nabla^2 p_s(x,y) & = \Re \left[ \left( \partial_x - i \partial_y \right) \left( \frac{A(x,y)}{B(x,y)} \right) \right] .
\end{align}
...
\item $b=0, H(x,y) = 1 - x^2 - y^2$ in a circular parabolic basin.
\item $b(x,y)$ given by solving the vertical diffusive buoyancy equation and $H(x,y) = 1 - x^2 - y^2$ in a circular parabolic basin.
\end{enumerate}
